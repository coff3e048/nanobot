...
class mediastuff(commands.Cog):
    ...
    async def ytdl(self, ctx, query: str = None, convert: str = "video"):
        ...
        if not query or query == None:
            ...
            #if query.startswith("https://youtu") == False:
            #    await ctx.reply("Not a YouTube URL. Only YouTube videos are allowed to be downloaded.")
            #else:
            ...
            if opts != None:
                ...
                try:
                    if exitcode == 0 and os.path.exists(tmplocation):
                        ...
                        await msg.reply(f"Something went wrong. The video download likely failed (and/or was too big) ```{exitcode}```")
                except:
                    await msg.reply(f"Something has gone wrong, and I don't know the error.")
